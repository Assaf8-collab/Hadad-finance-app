...
if bank_up and credit_up:
    ...
    # --- ניהול הכנסות אינטראקטיבי ---
    st.divider()
    st.subheader("🏦 הגדרת מקורות הכנסה")
    st.write("סמן בתיבת הבחירה את מקורות ההכנסה התזרימיים (משכורת וכו').")
    ...
    # הכנת טבלת אפשרויות
    income_options = df_inc_raw.groupby('תיאור התנועה')['סכום'].agg(['sum', 'count']).reset_index()
    income_options.columns = ['תיאור מקור ההכנסה', 'סכום מצטבר', 'פעמים']
    
    # השם כאן חייב להתאים בדיוק לשימוש בשורה של הסינון (נכלל)
    income_options.insert(0, "נכלל", income_options['תיאור מקור ההכנסה'].isin(previously_approved))

    edited_income = st.data_editor(
        income_options,
        column_config={
            "נכלל": st.column_config.CheckboxColumn("אישור", default=True),
            "תיאור מקור ההכנסה": st.column_config.TextColumn("תיאור מלא מהבנק", width="large"),
            "סכום מצטבר": st.column_config.NumberColumn("₪ סה\"כ", format="₪%.0f"),
        },
        disabled=['תיאור מקור ההכנסה', 'סכום מצטבר', 'פעמים'],
        hide_index=True,
        key="income_editor"
    )

    if st.button("💾 שמור הגדרות הכנסה לחודש הבא"):
        new_approved = edited_income[edited_income["נכלל"] == True]['תיאור מקור ההכנסה'].tolist()
        save_settings(new_approved)
        st.success("הבחירות נשמרו!")

    # סינון הנתונים
    approved_list = edited_income[edited_income["נכלל"] == True]['תיאור מקור ההכנסה'].tolist()
    df_inc_filtered = df_inc_raw[df_inc_raw['תיאור מקור ההכנסה'].isin(approved_list)]

    # --- חישוב הוצאות וסיכום ---
    credit_keywords = ['כ.א.ל', 'מקס', 'ישראכרט', 'חיוב לכרטיס', 'ויזה']
    df_bank_exp = df_bank[(df_bank['סכום'] < 0) & (~df_bank['תיאור התנועה'].str.contains('|'.join(credit_keywords), na=False))].copy()
    df_bank_exp['סכום'] = df_bank_exp['סכום'].abs()
    df_bank_exp['Month'] = df_bank_exp['תאריך'].dt.to_period('M')
    ...
    # איחוד חודשי וסינון חודשים שלמים
