...
def load_settings():
    # מבנה ברירת מחדל הכולל רשימת חסכונות
...
if bank_up and credit_up:
    ...
    # שלב המיון
    st.divider()
    st.subheader("🛠️ מיון וסיווג תנועות (בנק)")
    ...
    if available_months:
        ...
        if st.button("💾 שמור הגדרות"):
            # איסוף כל מה שסומן כאישור וכל מה שסומן כחסכון
            ...
            # עדכון הזיכרון (שמירה על הקיים ועדכון מה שהשתנה בטבלה הנוכחית)
    ...
    # החלת קטגוריות כולל זיהוי חסכון מה-UI
    ...
    if not summary.empty:
        ...
        st.subheader(f"🔍 ניתוח קטגוריות (בנק + אשראי + הכנסות שהן חסכון) - {last_m}")
        
        # איסוף כל התנועות שסווגו
        combined_all = pd.concat([
            df_c[df_c['Month'] == last_m][['קטגוריה', 'סכום']],
            df_exp_f[df_exp_f['Month'] == last_m][['קטגוריה', 'סכום']],
            df_inc_f[df_inc_f['Month'] == last_m][['קטגוריה', 'סכום']]
        ])
        
        # סינון רק לקטגוריות של הוצאות או חסכון (מתעלמים מהכנסות רגילות בגרף)
        cat_analysis = combined_all[combined_all['קטגוריה'] != 'אחר'].groupby('קטגוריה')['סכום'].sum().sort_values(ascending=False)
        
        c1, c2 = st.columns([2, 1])
        with c1: st.bar_chart(cat_analysis)
        with c2:
            if 'חסכון והשקעות' in cat_analysis:
                st.metric("סכום שנחסך", f"₪{cat_analysis['חסכון והשקעות']:,.0f}")
