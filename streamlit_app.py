...
def load_settings():
    default = {"approved_income": None, "approved_expenses": None}
...
def save_settings(income_list, expense_list):
    with open(SETTINGS_FILE, 'w', encoding='utf-8') as f:
        json.dump({"approved_income": income_list, "approved_expenses": expense_list}, f, ensure_ascii=False, indent=4)
...
CATEGORY_MAP = {
    ...
    'חסכון והשקעות': [ 'INTERACTIVE','הפקדה', 'חסכון', 'ניירות ערך', 'קופת גמל', 'פנסיה', 'השתלמות', 'פקדון', 'קופת'],
...
st.title("💰 ניהול תזרים מזומנים וחסכון")
...
if bank_up and credit_up:
    ...
    if available_months:
        selected_month = st.selectbox("בחר חודש לבדיקה וסיווג:", available_months)
        ...
        if st.button("💾 שמור בחירות"):
            save_settings(ed_inc[ed_inc["אישור"] == True]['מקור התנועה'].tolist(), ed_exp[ed_exp["אישור"] == True]['מקור התנועה'].tolist())
            st.success("ההגדרות נשמרו!")
            st.rerun()

    # --- סיכום וניתוח ---
    updated_settings = load_settings()
    df_inc_f = df_inc_raw[df_inc_raw['מקור התנועה'].isin(updated_settings['approved_income'])] if updated_settings['approved_income'] is not None else df_inc_raw
    df_exp_f = df_bank_exp_raw[df_bank_exp_raw['מקור התנועה'].isin(updated_settings['approved_expenses'])] if updated_settings['approved_expenses'] is not None else df_bank_exp_raw
    ...
    if not summary.empty:
        ...
        st.divider()
        st.subheader("📊 סיכום תזרים מזומנים")
        st.table(summary.sort_index(ascending=False).style.format("₪{:,.2f}"))

        # ניתוח קטגוריות כולל חסכון
        last_m = summary.index[0]
        st.subheader(f"🔍 ניתוח קטגוריות (בנק + אשראי) - {last_m}")
        all_exp = pd.concat([df_c[df_c['Month'] == last_m][['קטגוריה', 'סכום']], df_exp_f[df_exp_f['Month'] == last_m][['קטגוריה', 'סכום']]])
        cat_sum = all_exp.groupby('קטגוריה')['סכום'].sum().sort_values(ascending=False)
        
        c1, c2 = st.columns([2, 1])
        with c1: st.bar_chart(cat_sum)
        with c2: 
            if 'חסכון והשקעות' in cat_sum:
                st.metric("סכום שנחסך", f"₪{cat_sum['חסכון והשקעות']:,.0f}")
            st.write(cat_sum.map("₪{:,.2f}".format))
